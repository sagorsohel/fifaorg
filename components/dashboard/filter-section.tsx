    ...
    <section className="flex flex-col lg:flex-row justify-between items-stretch lg:items-center gap-3 bg-slate-955/20 p-3 rounded-2xl border border-slate-900/60 shadow-inner">
      ...
      <div className="flex p-1 bg-slate-950 rounded-xl border border-slate-900 shrink-0 w-full lg:w-auto">
        <button
          ...
          className={`flex-1 lg:flex-initial flex items-center justify-center gap-2 px-4 py-2 lg:px-6 lg:py-2.5 rounded-lg text-xs lg:text-sm font-semibold transition-all duration-200 cursor-pointer ${
            activeTab === "matches"
                  ...
                  {(["all", "finished", "upcoming"] as const).map((status) => (
                    <DropdownMenuItem
                      ...
                      className={`cursor-pointer px-3 py-2 text-xs rounded-lg transition-all focus:bg-cyan-500/15 focus:text-cyan-400 font-bold ${
                        filterStatus === status ? "bg-cyan-500/10 text-cyan-400" : "text-slate-300"
                      }`}
                    >
                      {status === "all"
                        ? translate("all_matches", lang)
